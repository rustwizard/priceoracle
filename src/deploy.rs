...
pub fn run_with_http(
    ...
    info!(
        logger,
        "deploy called to the {} network with {:?}", config.net, config.from_addr
    );
...
fn with_existing_wallet(
    ...
    info!(
        logger,
        "deploy contract from {:?} with suggested gas_price: {:?}", conf.from_addr, gas_price
    );
